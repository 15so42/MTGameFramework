...
namespace FIMSpace.FProceduralAnimation
{
    public partial class RagdollProcessor
    {
        ...
        public void TryAutoFindReferences(Transform root)
        {
            ...
            if (Pelvis == null) Pelvis = FindChildByNameInDepth("pelvis", root);
            if (Pelvis == null) Pelvis = FindChildByNameInDepth("hips", root);
            if (SpineStart == null) SpineStart = FindChildByNameInDepth("spine", root);
            if (Chest == null) Chest = FindChildByNameInDepth("chest", root);
            if (Head == null) Head = FindChildByNameInDepth("head", root);

            if (BaseTransform != null)
                if (LeftUpperArm == null)
                {
                    LeftUpperArm = FindChildByNameInDepth("arm", root);
                    if (LeftUpperArm != null)
                        if (BaseTransform.InverseTransformPoint(LeftUpperArm.position).x > 0.001f)
                            LeftUpperArm = FindChildByNameInDepth("left", root);
                }

            if (RightUpperArm == null) RightUpperArm = FindChildByNameInDepth("right", root);
            if (LeftUpperLeg == null) LeftUpperLeg = FindChildByNameInDepth("leg", root);
            if (RightUpperLeg == null) RightUpperLeg = FindChildByNameInDepth("leg", root);
        }
